...
router.post('/shopify', rateLimit_1.rateLimiter, async (req, res) => {
    try {
        ...
        const tenant = await (0, webhook_1.findTenantByDomain)(shopDomain);
        if (!tenant) {
            console.log(`Tenant not found for shop: ${shopDomain}`);
            return res.status(404).json({ error: 'Tenant not found' });
        }
        // Verify HMAC signature
        const isValid = (0, webhook_1.verifyHmac)(rawBody, hmacHeader, tenant.webhookSecret);
        if (!isValid) {
        ...
        await (0, webhook_1.routeWebhook)(tenant.id, topic, req.body);
